...
| ----      | ----------  | ----------  |
| -b        | Board      | Specifies the device code(s) to build. For example, `-b s905x3` builds the device with code s905x3. Multiple device codes can be connected using underscores, such as `-b s905x3_s905d`. Special values: `all` builds all devices; `top50` builds the top 50 devices in the device list; `rest50` builds from the 51st device to the end. The full list of device codes can be found in the `BOARD` entries of [model_database.conf](build-armbian/armbian-files/common-files/etc/model_database.conf). Default: `all` |
